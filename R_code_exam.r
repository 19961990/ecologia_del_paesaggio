...
install.packages("sp")# facciamo dialogare R con l'esterno , il pachetto sp svillupa per i dati spaziali JK
# require(sp)è un altro comando per fare partire le librerie JK
library(sp) # è relazionato a dati spaziali . ()  si usa perche R richiede spiegazioni dell'argomento JK
data(meuse) #data è il datum, meuse è il dataset a l'interno del paccheto sp
meuse
...
pairs(meuse) # la funzione pairs mostra un grafico con tutte le variabili sulla diagonale del grafico e vari plot che che vediamo sono le correlazione in modo grafico tra le varie variabili JK
...
#metodo per fare il subset di un database in R si chiama dataframe ,selezionando alcuni variabili,la funzione pairs è stata riscritta.JK
#mettiamo il data set meuse e quali colonne ci interessa JK
#con a funzione names il cadmio, il rame,il piombo e lo zinco erano alla terza, quarta,quinta e sesta variabili quindi si fa un meuse da 3 a 6 JK
pairs(meuse[,3:6]) #si mettono le funzioni dentro la parentesi tonda JK
# cambiamento di colori dei punti JK
pairs(meuse[,3:6], col="red") # la funzione col si usa per colore. red si mette tra "" perche è un etichetta JK
#cambiare il carattere dei punti a l'interno del grafico JK
...
#per aumentare la dimensione o ridurre  i point character  si usa  character exageration cex JK
#se cex=1 rimane uguale, se cex=2 ,significa 2 volte più grande,se cex=0,1 riduzione di metà JK
...
# {points(x, y, pch = pch, col = col, bg = bg, cex = cex)ok <- is.finite(x) & is.finite(y)
#  if (any(ok))lines(stats::lowess(x[ok], y[ok], f = span, iter = iter),col = 1, ...)}
#la funzione
...
# 2 R CODE SPATIAL.r
#R spatial  sono funzioni spaziali in ecologia del paesaggio
install.packages("sp") 
library (sp)
head(meuse)
data(meuse)
...
#invertiamo grafici rigga/colonna con colonna/ rigga
#davvanti a serie di numeri e caratteri si usa la c in R(numero di righe,numero di colonne)JK
#è stato creato un multipanel con la funzione "par",fatto da una righa e 2 colonne ,in queste 2 colonne abbiamo inserito un plot del cadmio e piombo e nella seconda righa un plot di rame e zinco JK
par(mfrow=c( 2,1))
plot(cadmium,lead,col="red",pch=19,cex=2)
plot(copper,zinc,col="green",pch=17,cex=2)

#multigrame automatico   
install.packages("GGally") #estensione di ggplot2, for correlation matrix and survival plots, R software and data visualization jk
library(GGaly) # richiamare con la libreria
ggpairs(meuse[3:6]) #ggplor2 matrix of plot JK
#spatial
head(meuse)#il dataset è meuse
#spieghiamo al software che abbiamo bisogno di utilizzare le coordinate JK
#in R  il dataset si chiama dataframe
...
#funzione per plottare dati spaziali "plot"
plot(meuse)
#funzione spplot per plottare i dati raster o altri ogetti spazialmente è una funzione dentro la libreria sp JK
spplot(meuse,"zinc")
...
install.packages ("sp") #serve per dati spaziali punti,linee,poligoni
...
head(meuse) # con head si da un occhiata al dataset ed è la prima parte del database cioè le 6 prime righe JK
#un altra possibilità per vedere i nomi delle colonne  JK 
names(meuse) #funzione "names" permette di vedere i nomi JK
spplot(meuse,"copper")

#bubble è una funzione del pacchetto sp,è come "spplot" cioè un modo per plottare i dati e permette di avere grafici più carini JK
bubble(meuse,"zinc")# meuse è il dataset di sp e zinco la variabile ed è dichiarato con le ""
                              
#exercise bubble del rame colorato rosso
                              
bubble(meuse,"copper",col="red") #red è una stringa di testo per questo è tra "" jk
#esercizio
#ammettiamo che Sofia a da campionare i foraminiferi a terra in varie zone (6) e amettiamo che un certo plot abbiamo  misurato la quantità di foraminiferida 0 a 100
#quindi nel primo sarà 10 foraminiferi,nel secondo 20,nel terzo 35,nel quarto 50.nel quinto 67,nel sesto 80 in R.
...
carbon <- c(5, 15, 30, 70, 85, 99) # <- verso foraminiferi permette di nominarlo JK
...
#si crea una cartella chiamata "Lab" dove mettiamo tutto quello che carichiamo nel computer e con quale lavoreremo nel corso JK
...
#covid_agg.csv è tra "" perché si va a l'estero del software



4 R CODE POINT PATTERN.r
                              
#CODICE PER ANALISI  DEI POINT PATTERNS
                              
#l'analisi del point pattern permette di stimare i valori che non possiamo misurare
install.packages("ggplot2") # mpg è uno dei dataset del pacchetto ggplot 2 JK
...
plot(covid$country,covid$cases,las=0) #parallele labels. il "$" o attach covid permette di collegare un pezzo a un altro JK
...
#ggplot per visualizzare la distribuzione spaziale del covid,names permette di vedere dati di covid JK
...
#bisogna inserire le coordinate x,y e il rangedentro al dataset -180 a +180 long e -90 a 90 latt JK
covidsppp<-(lon,lat,c(-180,180),c(-90,90))# c serve davvanti a numeri
#prima di lanciare la funzione facciamo attach JK
attach(covid) # covid è il dataset originale JK
library(spatstat) #il package spatstat analise spatial point pattern JK
d<- density(covids)
plot(d) # d è la densità jk
points(covids) > ppp(x.coordinates,y.coordinates,x.range,y.range) #covids è il nuovo dataset che useremo per la parte spaziale della densità JK
#ppp è una funzione,è il dataset for spatstat. JK
#plot della densità
plot(d)
...
plot(d,col=cl) # ll l'ogetto,è la colorRampe palette.
...
#excercise plot della mappa di densità con una nuova colorazione  e aggiunta delle coastlines
...
#installare il pacchetto spatstat e chiamare la libreria rgdal
library(spatstat)
library(rgdal) # for the coastlines
setwd("C:/lab") #set working directory
#inserire tutti i dati che abbiamo salvato tra cui c'è d,plottare la densità usando il colorRampe palette.poi si rinomina il nome density con"main" JK
load("point_pattern.RData") #per aggiungere punti si usa points pattern  JK
ls() #per vedere la lista jk
#point pattern non aveva ancora salvato le coastlines  quindi qui c'è una bifurcazione JK
cl5 <- colorRampPalette(c('cyan', 'purple', 'red'))(200) # color rampe palette stabilisce la legenda JK
plot(d, col=cl5, main="density")
#aggiunto al precedente plot punti con points con covids come point pattern creato con la funzione ppp jk
points(covids)
...
plot(coastlines, add=T)  
#andiamo a vedere la densità dei paesi che hanno avuto il covid uno accanto a l'altro con l'interpolazione JK
#interpolazione va a stimare i valori nelle zone dove non è stato fatto campione JK
...
view(covid) o covid #per vedere tutta la tabella e creare dei valori di interpollazione  JK
#marks o voto sono delle etichette per ogni paese JK
...
#exercice plot(S) fare un plot(s) cambiando la palette e aggiungendo i punti coastlines  e il titolo main

#la mappa si chiama interpol e lo associamo con  <- alla funzione smooth sul nostro point pattern e lo incolliamo su R
plot(dT, main="Density of points")
points(Tesippp,col="green")
plot(interpol, main="Estimate of species richness")
points(Tesippp,col="green") 
#la colorRampepalette
# densità JK                              
...
# creare la mappa di densità
dT <- density(Tesippp)
dev.off() # serve per chiudere la mappa
plot(dT)
points(Tesippp, col="green")
...
plot(dT)
points(Tesippp, col="green")    
head(Tesi) #la funzione head serve per leggere la tabella Tesi jk
...
plot(interpol)
points(Tesippp, col="green")
library(rgdal)#  il pachetto rgdal serve PER APRIRE QUALSIASI FILE VECTORIALE JK
#associamo al nome san marino la funzione read OGR jk
sanmarino <-  readOGR("San_Marino.shp")
# plot del POLIGONO JK
plot(sanmarino)
plot(interpol, main="Species richness", add=2) #così si aggiungono pezzi alla mappa precedente jk
points(Tesippp,col="green")
#per avere la mappa dei punti,per plottare sopra la mappa precedente JK
plot(sanmarino, add=2)#è la mappa finale jk
...
#si è carica il zip da iol e si mette nella cartella lab  JK 
#codice R per analisi immagini satellitari JK
#packages raster JK
install.packages("raster")
library (raster) #il pachetto raster serve per importare i dati dentro R
setwd("C:/lab/")
...
plot(p224r63_2011$B1_sre, col=clb) #$ permette di collegare vari pezzi di oggetto,lo avevamo usato per collegare le colonne a loro dataset o le bande a loro dataset cioè a l'immagine JK
...
#esercizio: plottare la banda del l infrarosso vicino con colorrampalette che varia dal rosso all arancione al giallo 
                              
clnir <- colorRampPalette(c('red','orange','yellow'))(100) 
plot(p224r63_2011$B4_sre, col=clnir)
#  multiframe di 4 bando 
# blue
clb <- colorRampPalette(c('dark blue','blue','light blue'))(100) 
...
clnir <- colorRampPalette(c('red','orange','yellow'))(100) 
plot(p224r63_2011$B4_sre, col=clnir) 
 dev.off() #per chiudere i  GRAFICI JK
...
# B3: red
# B4: near infrared (nir)
# plotRGB(p224r63_2011,r=3,g=2,b=1) vedere i colori come a occhio nudo sul immagine satellitare  j
    
plotRGB(p224r63_2011, r=3, g=2, b=1)
plotRGB(p224r63_2011, r=3, g=2, b=1, stretch="Lin") #strecth=Lin  funzione per tirare JK
#nir 
# false colour jk
...
#excercise : near infrared (nir) nella componente green si vede la parte della vegetazione.la parte nuda del suola viola.G green
...
p224r63_1988 <- brick("p224r63_1988_masked.grd")  #brick per importare le immagini satellitari tutti insieme jk
plot(p224r63_1988)
# multiframe
par(mfrow=c(2,2))
# blue
...
# nir
clnir <- colorRampPalette(c('red','orange','yellow'))(100) # near infrared (nir)
...
# per collegare data set a l immagine si usa il $, si usa <- per ridurre un ogetto a un altro cosi senza parantesi dopo, sre è il sensore JK
...
dvi1988 <- p224r63_1988$B4_sre-p224r63_1988$B3_sre #$ si usa per legare ogni singola a un dataset in questo caso le bande a l'immagine JK
...
#abbiamo usato appena un immagine che pixel per pixel  è la sostrazione dell' IR vicino e del Rosso se la pianta è sana il valore sarà alto JK
...
plot(dvi1988)#plot serve per la rappresentazione grafica su R
plot(dvi2011, col=cldvi)
                              
# multitemporal analysis
#analisi multitemporale  servono per mostrare vegetazioni che hanno subito più o meno stress  JK 
#facciamo la differenza di dvi che è dif dvi lo assegniamo con  <- la div di 2011 e 1988 JK
...
plotRGB(p224r63_1988, r=4, g=3, b=2, stretch="Lin") #lin=lineare,stretch=tirare jk
plotRGB(p224r63_2011, r=4, g=3, b=2, stretch="Lin")
plot(difdvi, col=cldifdvi)  
dev.off() 
                              
#modificare la grana o resoluzione con la funzione "aggregate" con cui possiamo aggregare i vari pixel e creamo un immagine con pixel più grande JK
...
#definizione di classi aumenta l incertezza del'algoritmo  automatico di classificazione
...
library(RStoolbox)
#RStoolbox: Tools for Remote Sensing Data Analysis Toolbox for remote sensing image processing and analysis such as calculating spectral indices, principal component transformation, unsupervised and supervised classification or fractional cover analyses jk
#defor1_.1=NIR
#defor1_.2=RED
#defor1_.3=GREEN
...
par(mfrow=c(2,2))plotRGB(defor1, r=1, g=2, b=3, stretch="Lin")plotRGB(defor2, r=1, g=2, b=3, stretch="Lin") #"par"serve per il plot di 2 oggetti insieme JK
...
plot(defor1_c$map)
cl <- colorRampPalette(c('darkblue','blue','light blue'))(100) # facciamo un color rampe palette Per visualizzare JK
...
#PERCENTUALI si mette su R percent1 per calcolare. percentuale della mappa è uguale alla frequenza delle mappe per 100 diviso tot1 JK
#FORESTE=10.79047
#aree =89.20953
...
#nella seconda colonna sono i valori  di agricoltura per foresta nella prima mappa chiamata defor prima della deforestazione.il c serve prima di mettere 2 valori nelle parantesi JK
#after è per l'altra colonna dopo la deforestazione  e inseriamo 2 valori a l'interno delle paratesi JK 
before <- c(10.7,89.20)
after <- c(47.67,52.32)
                              
#creare un  dataframe finale o output con valore di foresta e aree aperta e le associamo al dataframe con una funzione che è data.frame JK
...
library(ggplot2)
#andiamo a fare un grafico basato sulla libreria ggplot2,il dataset è output,cioè il dataset creato l'ultima volta,poi un dataframe con la copertura,la percentuale di prima deforestazione e dopo JK
...
ggplot(output, aes(x=cover, y=after, color=cover)) + # x la copertura prima e y la copertura dopo
geom_bar(stat="identity", fill="white")
dev.off() 
                              
#nuovo grafico:plot dei 2 histogrammi insieme .la funzione par non funziona con ggplot2 quindi bisogna installare un altro pachetto che si chiama "gridExtra" JK 
install.packages("gridExtra")
library(gridExtra) o require(gridExtra)
#plot finale con tutti dati a l'interno e usiamo le funzione "grid.arrange" che va a prendere vari plot e le mette insieme a l'interno dello stesso grafico quindi gioca il ruolo di "par" JK
...
plot(EN013)
#si può anche usare la funzione "brick"
...
#per vederle insieme si usa la funzione "par" JK
par(mfrow=c(1,2))
plot(EN01, col=cl)
plot(EN13, col=cl)
...
#stabiliamo un nuovo colorRampe palette o cl
...
#andamo a prendere list.rast per farlo basandosi su raster rinominato r
...
setwd("C:/lab/") 
#importiamo dati che sono valori  di N02 da gennaio a marzo 
setwd("C:/lab/esa_NO2")
#usiamo le cartelle esano2 oltre alla cartella lab a l'interno della cartella lab ci sono dei file che sono tutti .png JK
...
snowmay <- raster("c_gls_SCE500_202005180000_CEURO_MODIS_V1.0.1.nc")#tra le virgolette c'è l'immagine,snoway serve a....
...
# writeRaster(d2c$map,"d2c.tif") # la mappa è collegta al processo di classificazione con il $
#con writeraster sono stati scritti questi file a l'interno della cartella lab
#per caricare i dati abbiamo 2 funzioni  brick per caricare tutte le bande o raster per un solo bando JK,
...
cl <- colorRampPalette(c('green','black'))(100) 
plot(d1c,col=cl) #colore=cl è il colorRampe palette jk
plot(d2c,col=cl)
...
#ognuno classificata estraemo solo la foresta perche dopo faremo dei calcoli che riguarda la foresta e vogliamo andare a estrarre la foresta per fare questi calcoli JK
#per farlo basta annullare tutto quello che non è foresta e la funzione è "cbind" che elimina alcuni valori e li mettiamo con i non valori NA jk
...
#abbiamo assegnato il valore 2 alla foresta ,riclassifichiamo solo la mappa per estrarre solo la mappa jk
...
#presentare la mappa con un "par"
par(mfrow=c(1,2))
cl <- colorRampPalette(c('black','green'))(100) #
plot(d1c.for,col=cl)
plot(d1c.for) 
...
#creare i patch:unire tutti i pixel vicino per creare ogni singola patch jk
#ragrupare i pixel vicini in una singola patch useremo la funzione "clump" che aggrega pixel vicini e gli mette come singoli patch jk
...
# plot results con la libreria ggplot2 con variazione di patch nel tempo
time <- c("Before deforestation","After deforestation")
npatches <- c(568,1920)
...
setwd("C:/lab/snow/") #fare un set working directory della
...
# fare lo zoom sull'immagine e un crop jk
#usiamo la funzione "zoom" e il nome dell'immagine più l'estenzione JK 
#si definisce prima l'estenzione di un certo file mettendo ad esempio il numero dell'estenzione xmax,xmin,ymax,ymin jk
#si ritaglia l'immagine in base a l'estenzione definito da noi  jk    

#plot di una delle immagine legiamo il nostro stack files attraverso il $ a l'immagine che vogliamo plottare    jk
...
zoom(snow.multitemp$snow2010r, ext=extension) #zoom più nome dell'immagine e l'estenzione jk
...
#plot della nuova immagine con il crop fatto prima.i range delle varie legenda le mettiamo tutti uguali jk
#vediamo i valori minimi e e massimi per tutte le legende e mettiamo il nome "snow mutitemp.italy"     
...
#boxplot in horizontale o previsione con la funzione prediction,il boxplot permette di prendere i valori intermedi e variazione dei valori di copertura nevosa.JK
...
library(sdm)     
library(raster)   
library(rgdal) #permette di gestire al meglio dati raster e vettoriali a l'interno del software  jk
#carichiamo il file a l interno del pachetto sdm jk
#shp è un file del sistema quindi usiamo la funzione system.file, carichiamo il file esterno jk
#digitiamo system file e la mettiamo all'interno di R  jk   
...
#caricare la parte grafica con la funzione shapefile ,così possiamo usare quel dato per mappare quello che è la distribuzione delle specie allo stato attuale JK   
#[1] "C:/Users/HP/Documents/R/win 
#il dataset è formato da 2 punti con delle coordinate,una certa area di studio,e ognuno dei punti è relazionato a l'interno di una tabella che raccoglie dati di occurence JK 
...
#A l'interno della cartella c'è una serie di files e si usa list.files,il pattern è asc che è un formato jk
lst <- list.files(path=path,pattern='asc$',full.names = T) # T =true
#usiamo il codice per importare i vari predictori che sono delle variabili ambientali che servono a prevedere quello che sarà la distribuzione della specie nello spazio JK
#quindi le variabili sono le precipitazione,indice di vegetazione,la temperatura  jk
#facciamo uno stack di tutte le variabili.diamo un nome commune preds che sono quelle variabili che sono usate per prevedere quello che sarà la distribuzione della specie       
...
plot(preds, col=cl)
#plot di una singola variabile elevation legato al preds col $     
plot(preds$elevation, col=cl)
#mettiamo sopra i punti con la funzione point con occorenze uguale a 1  jk  
...
#usiamo la colorRampe palette precedente cl    
plot(p1, col=cl)
...
#il pattern potrebbe essere SWI .nc   
library(sp) # è relazionato a dati spaziali . ()  si usano perche R richiede spiegazioni dell'argomento JK
#richiamare la libreria ncdf4      
library (ncdf4) #per utilizzare i dati nc da copernicus JK        
#richiamare library(raster)
library(raster) #importiamo e singole immagini con la funzione raster       
#vediamo la lista di file con la funzione rlist ,con tutti file che finiscono con ".nc"      
rlist <- list.files(pattern=".nc")  #in questo caso  .nc e il pattern   
rlist=list.files(pattern=".nc", full.names=T)      
rlist #  
#si può importare un immagine in R uno dopo l'altro con raster      
SWI2015 <- raster("c_gls_SWI1km_201505221200_CEURO_SCATSAR_V1.0.1.nc")
plot(SWI2015,main="soil water index Europe 2015") #la funzione main serve per cambiare il titolo del grafico
SWI2020 <- raster("c_gls_SWI1km_202005211200_CEURO_SCATSAR_V1.0.1.nc")      
plot(SWI2020,main="soil water index Europe 2020") # il grafico dell' indice del suolo nel 2020 JK
     
#  o con brick brick serve a importare immagini satellitare completamente in tutte le bande a l'interno di R       
...
plot(SWI2020) 
#con brick abbiamo gli stessi grafici di prima con raster 
     
#importare tutte le immagine insieme
#con un ciclo for si dice al sistema di prendere tutte le immagine  insieme con la funzione stack 
#caricare tutti i file con ciclo for:                       
list_rast <- lapply(rlist, raster) # lapply va applicare la funzione di riferimento a l'interno della lista di file JK     
...
#plot dell'immagine di maggio 2015 
plot(SWI2015, col=cl) 
clb <- colorRampPalette(c('blue','pink'))(100) 
#plot dell'immagine di maggio 2020    
...
plot(swi.multitemp$Surface.State.Flag.1,col=cl)  #colorRampe palette di 2015 
swi.multitemp #con questo comando in R Vediamo la risoluzione ,le coordinate e l'estenzione
names # con la funzione names vediamo il nome dei vari file
...
zoom(swi.multitemp$Surface.State.Flag.1, ext=extension)
zoom(swi.multitemp$Surface.State.Flag.1, ext=drawExtent())#estenzione=drawExtend
...
Surface.State.Flag.1.italy<- crop(swi.multitemp$Surface.State.Flag.1, extension)  #la funzione crop fa una nuova immagine 
plot(Surface.State.Flag.1.italy, col=cl)
                       
#PER 2020 
extension <- c(5, 35, 35, 50)
Surface.State.Flag.2.italy<- crop(swi.multitemp$Surface.State.Flag.2, extension)  #la funzione crop fa una nuova immagine
plot(Surface.State.Flag.2.italy, col=clb)
     
#facciamo un immagine dell'acqua nel suolo dell' italia a scala europea per le 2 immagini cioè co l'estensione di origine delle immagini
#per 2015
...
#per 2020
extension <- c(-11, 40, 35, 72) 
Surface.State.Flag.2.italy<- crop(swi.multitemp$Surface.State.Flag.2, extension) 
plot(Surface.State.Flag.2.italy, col=clb)      
...
# da quello che osservo tra 2015 e 2020 non c'grande variazione dell'indice dell'acqua nel suolo sia in Europa che in Italia.
